        ...
        <div class="content-wrapper">
          ...
          <div class="d-flex justify-content-center py-0 py-md-4">
            <PaginationComponent
              ...
              :total-pages="tagStore.totalPages"
...
export default {
  ...
  computed: {
    ...
    visiblePages() {
      ...
      let pages = [];
      const total = this.tagStore.totalPages;

      // 현재 페이지에서 앞뒤로 2개씩 보이게 하되, 총 페이지 수를 초과하지 않도록 조정
      let start = Math.max(1, this.tagStore.currentPage - 2);
  ...
  methods: {
    changePage(page) {
      this.tagStore.getTagList(page);
    },
    jumpForward() {
      // 현재 페이지에서 3페이지 앞으로 점프
      let nextPage = Math.min(
        this.tagStore.currentPage + 3,
        this.tagStore.totalPages
      );
      this.changePage(nextPage);
      // visiblePages를 업데이트하기 위해 currentPage를 설정
      this.tagStore.currentPage = nextPage;
    },
  },
};
</script>
