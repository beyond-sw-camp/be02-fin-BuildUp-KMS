<template>
  <td>
    <span class="fw-medium">{{ boardCategories.idx }}</span>
  </td>
  <td>{{ boardCategories.categoryName }}</td>
  <td>
    <div class="dropdown" @click.prevent="toggleUserMenu($event)">
      ...
      <div class="dropdown-menu" v-show="userMenuVisible">
        <router-link
          :to="{ name: 'AdminBoardCategoryUpdate', params: { categoryIdx: boardCategories.idx, categoryName: boardCategories.categoryName } }">
          <a class="dropdown-item"><i class="bx bx-edit-alt me-1"></i>수정</a>
        </router-link>

        <a class="dropdown-item" @click="deleteBoardCategory"><i class="bx bx-trash me-1"></i>삭제</a>
      </div>
    </div>
  </td>
</template>
...
export default {
  name: 'AdminBoardCategoryListComponent',
  props: {
    boardCategories: Object
  },
  setup(props) {
    ...
    const deleteBoardCategory = async () => {
      try {
        await boardStore.deleteBoardCategory(props.boardCategories.idx);
        alert('게시판 카테고리가 삭제되었습니다.');
      } catch (error) {
        console.error('게시판 카테고리 삭제 에러:', error);
        alert('게시판 카테고리 삭제에 실패했습니다.');
      }
    };
    ...
    return { userMenuVisible, toggleUserMenu, deleteBoardCategory };
  },
  computed: {
