<template>
  <td>
    <span class="fw-medium">{{ users.userIdx }}</span>
  </td>
  <td>{{ users.email }}</td>
  <td>{{ users.name }}</td>
  ...
  <td>
    <div class="dropdown" @click.prevent="toggleUserMenu($event)">
      ...
      <div class="dropdown-menu" v-show="userMenuVisible">
        <a class="dropdown-item" @click="deleteUser"><i class="bx bx-trash me-1"></i> 삭제</a>
      </div>
    </div>
  </td>
</template>
...
export default {
  ...
  setup(props) {
    ...
    return { userMenuVisible, toggleUserMenu, deleteUser };
  },
  computed: {
