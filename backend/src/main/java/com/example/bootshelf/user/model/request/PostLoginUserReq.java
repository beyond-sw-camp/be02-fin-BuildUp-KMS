...
public class PostLoginUserReq {
    ...
    @Length(min=8, max=45, message = "패스워드는 8글자 이상, 20글자 이하여야 합니다.")
    @Pattern(regexp="^(?=.*[A-Za-z])(?=.*\\d)(?=.*[@$!%*#?&])[A-Za-z\\d@$!%*#?&]{8,45}$", message = "패스워드는 대/소문자, 특수문자, 숫자를 반드시 포함한 8글자 이상이어야 합니다.")
    @ApiModelProperty(value = "패스워드(최소 : 8글자, 최대 45글자 / 적어도 한개의 대/소문자, 특수문자, 숫자를 포함)", example = "Qwer1234@", required = true)
